...
namespace mcts_checkers {
    ...
    static std::pair<std::vector<AttackAction>, uint64_t> collect_attacks(const CheckersData& data, uint8_t checker_index);

    template<bool is_king>
    static auto block_traversal_general(
        const int8_t block_x, const int8_t block_y,
        const CheckersData& data,
        const bool checker_player,
        const uint8_t checker_index,
        const uint8_t enemy_checker_index,
        std::vector<AttackAction>& actions,
        std::vector<uint64_t>& action_sizes
    ) {
        ...
        new_data.m_is_king.set(block_checker_index, data.m_is_king[checker_index]);

        auto [action, action_size] = collect_attacks<is_king>(new_data, block_checker_index);
    ...
    template<bool is_king>
    static void enemy_traversal(
        const int8_t x, const int8_t y,
        const int8_t dev_x, const int8_t dev_y,
        const int8_t bound_x, const int8_t bound_y,
        const CheckersData& data,
        const bool checker_player,
        const uint8_t checker_index,
        std::vector<AttackAction>& actions,
        ...
        if(
            ...
            if constexpr (is_king) {
                while(block_y != bound_y and block_x != bound_x) {
                    const auto is_blocked = block_traversal_general<true>(
                        block_x, block_y, data, checker_player, checker_index,
                        enemy_checker_index, actions, action_sizes
                    );
                    if(is_blocked) break;
                ...
                if(block_y != bound_y and block_x != bound_x) {
                    block_traversal_general<false>(
                        block_x, block_y, data, checker_player, checker_index,
                        enemy_checker_index, actions, action_sizes
                    );
                }
    ...
    template<bool is_king>
    static std::pair<std::vector<AttackAction>, uint64_t> collect_attacks(
        const CheckersData& data,
        const uint8_t checker_index
    ) {
            ...
            for(const auto [bound_x, dev_x] : DEVIATIONS) {
                ...
                if constexpr(is_king) {
                    while(y != bound_y and x != bound_x) {
                        enemy_traversal<is_king>(
                            x, y, dev_x, dev_y, bound_x, bound_y,
                            data, checker_player, checker_index,
                            actions, action_sizes
                        );
    ...
    std::pair<std::vector<AttackAction>, uint64_t> collect_pawn_attacks(const CheckersData& data, const Vector<uint8_t> checker_board_vector) {
        return collect_pawn_attacks(data, convert_board_vector_to_checker_index(checker_board_vector));
    }
    std::pair<std::vector<AttackAction>, uint64_t> collect_pawn_attacks(const CheckersData& data, const uint8_t checker_index) {
        return collect_attacks<false>(data, checker_index);
    }
    ...
    std::pair<std::vector<AttackAction>, uint64_t> collect_king_attacks(const CheckersData& data, uint8_t checker_index) {
        return collect_attacks<true>(data, checker_index);
    }
